...
use rsa::{RsaPrivateKey, pkcs1v15::SigningKey, signature::RandomizedSigner};
...
impl SshAgentDaemon {
    pub async fn start(nyxdb: &Arc<Mutex<NyxDb>>) -> Result<(), Error> {
        ...
        #[cfg(unix)]
        {
            let agent_socket_path =
                std::env::var("SSH_AUTH_SOCK").unwrap_or_else(|_| "/tmp/nyx-agent.sock".to_string());
        ...
        #[cfg(windows)]
        {
            let pipe_name = std::env::var("SSH_AUTH_SOCK").unwrap_or_else(|_| r"\\.\pipe\nyx-agent".to_string());
    ...
    fn find_private_key(&self, request: &SignRequest) -> Result<PrivateKey, AgentError> {
        let db = match self.nyxdb.lock() {
            Ok(r) => r,
            Err(_) => {
                return Err(AgentError::other(crate::Error::Generic(
                    "Unable to lock database".to_string(),
                )));
            }
        };
        ...
        let privkey = PrivateKey::from_openssh(&key.private_key).map_err(AgentError::other)?;

        Ok(privkey)
    }

    fn determine_algorithm(
        &self,
        privkey: &PrivateKey,
        request: &SignRequest,
    ) -> Result<(HashAlg, String), AgentError> {
        // Get algorithm
    ...
    fn sign_rsa(&self, privkey: &PrivateKey, request: &SignRequest, alg: HashAlg) -> Result<Signature, AgentError> {
        ...
        let sig = Signature::new(ssh_key::Algorithm::Rsa { hash: Some(alg) }, sig_bytes).map_err(AgentError::other)?;
...
impl Session for SshAgentDaemon {
    ...
    async fn sign(&mut self, request: SignRequest) -> Result<Signature, AgentError> {
        ...
        let sig: ssh_key::Signature = match privkey.key_data() {
            ...
            _ => Signer::try_sign(&privkey, &request.data).map_err(AgentError::other)?,
        };
