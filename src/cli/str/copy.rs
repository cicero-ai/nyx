...
impl CliCommand for CliStrCopy {
    ...
    fn help(&self) -> CliHelpScreen {
        let mut help = CliHelpScreen::new(
            "Copy String",
            "nyx str cp <SOURCE> <DEST>",
            "Copy string to a new location",
        );

        help.add_param("SOURCE", "The source entry to copy from.");
        help.add_param("DEST", "The destination to copy the entry to.");

        help.add_example("nyx str cp mysite/pass1 anothersite/pass2");
        help
    }
}
