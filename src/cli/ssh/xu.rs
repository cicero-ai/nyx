...
impl CliCommand for CliSshKeyXu {
    fn process(&self, req: &CliRequest) -> anyhow::Result<()> {
        ...
        // Copy to clipboard
        clipboard::copy(&ssh_key.username)?;
    ...
    fn help(&self) -> CliHelpScreen {
        let mut help = CliHelpScreen::new(
            "Copy SSH Username",
            "nyx ssh xu <NAME>",
            "Copy SSH key username to clipboard",
        );

        help.add_param("NAME", "Name of entry to copy from.");
        help.add_example("nyx ssh xu mysite/cloudflare");
        help
    }
}
