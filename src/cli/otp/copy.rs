...
impl CliCommand for CliOtpCopy {
    fn process(&self, req: &CliRequest) -> anyhow::Result<()> {
        ...
        // Copy item
        rpc::send::<String, bool>("otp.copy", &vec![req.args[0].to_string(), req.args[1].to_string()])?;
    ...
    fn help(&self) -> CliHelpScreen {
        let mut help = CliHelpScreen::new(
            "Copy OTP Entry",
            "nyx otp cp <SOURCE> <DEST>",
            "Copy OTP entry to a new location",
        );

        help.add_param("SOURCE", "The source entry to copy from.");
        help.add_param("DEST", "The destination to copy the entry to.");

        help.add_example("nyx otp cp mysite/pass1 anothersite/pass2");
        help
    }
}
