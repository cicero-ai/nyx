...
impl CliCommand for CliOtpXp {
    fn process(&self, req: &CliRequest) -> anyhow::Result<()> {
        ...
        // Copy to clipboard
        clipboard::copy(&otp.secret_code)?;
    ...
    fn help(&self) -> CliHelpScreen {
        let mut help = CliHelpScreen::new(
            "Copy OTP Entry Secret Code",
            "nyx otp xp <NAME>",
            "Copy OTP entry secret code to clipboard",
        );

        help.add_param("NAME", "Name of entry to copy from.");
        help.add_example("nyx otp xp mysite/cloudflare");
        help
    }
}
